...
- (void) setDefaults {
    ...
    //Devika
    relatedURLs = [[NSMutableArray alloc]init];
...
- (void)mouseDown: (NSEvent *) theEvent {
    ...
    //devika - if the close circle is clicked, need the tab that it was associated with to delete
     NSPoint shiftedPoint = NSMakePoint(mousePointInView.x - 30, mousePointInView.y);
    //end devika
    ...
    //devika, remove the layer that contained the close Circle
    if ([clickedLayer.name  isEqual: @"closeCircle"]) {
        
        shiftedPoint = [self.layer convertPoint:shiftedPoint toLayer:tabsLayer];
        CALayer *delclickedLayer = [tabsLayer hitTest:shiftedPoint];
        if (delclickedLayer) {
            deleteThisTab = delclickedLayer;
        }
        //[self removeTab:delclickedLayer];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"SFTabDeleteNotification" object:self];
    ...
    if (clickedLayer &&  clickedLayer != tabsLayer ) {
        ...
        if (shouldSelectTab) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"SFTabChangedNotification" object:self];
        }
...
- (void) addTabAtIndex: (int) index withRepresentedObject: (id) representedObject {
    ...
    //Devika
   NSString *defaultURL = @"http://www.yahoo.com";
    
    [relatedURLs insertObject:defaultURL atIndex:index];
...
- (void) saveTabURL:(CALayer *) tab withURL: (NSString *) passURL {
    int tabIndex = [self indexOfTab:tab];
    if (tabIndex != -1) {
        [self saveTabURLAtIndex:tabIndex withURL:passURL] ;
    }
}


- (void) saveTabURLAtIndex: (int) index withURL: (NSString *) passURL {
   [relatedURLs insertObject:passURL atIndex:index];
}




- (void) addButtonWithRepresentedObject: (id) representedObject {
    
    [self addButtonAtIndex:[self numberOfTabs] withRepresentedObject:representedObject];
}

- (void) addButtonAtIndex: (int) index withRepresentedObject: (id) representedObject {
    // Loading the class that will render the tab layer.
    Class tabLayerClass = [[NSBundle mainBundle] classNamed: defaultTabClassName];
    id newtab = [tabLayerClass layer];
...
/*- (void) getTabURL:(CALayer *) tab {
    int tabIndex = [self indexOfTab:tab];
    if (tabIndex != -1) {
        [self getTabURLAtIndex:tabIndex];
    }
}*/


- (NSString *) getTabURL: (CALayer *) tab {
    int tabIndex = [self indexOfTab:tab];
    if (tabIndex != -1) {
        return [self getTabURLAtIndex:tabIndex];
    }
    return nil;
}

- (NSString *) getTabURLAtIndex: (int) index {
    return [relatedURLs objectAtIndex:index];
}
...
- (void) selectNextTab: (id) sender {
    int currentTabIndex = [ self indexOfTab: [self selectedTab] ];//Devika removed "unsigned"
...
//begin devika
- (CALayer *) tabToDelete {
    return deleteThisTab;
}
//devika


//begin devika
- (void) updateTabLabel: (CALayer *) tab withLabel: (NSString *) label {
    
    if (![arrangedTabs containsObject:tab]) {
        return;
    }
    currentSelectedTab = tab;
    [(id)currentSelectedTab setBrowserLabel:label];
    
}

//Devika

//- (NSURL *) selectedTabURL {
