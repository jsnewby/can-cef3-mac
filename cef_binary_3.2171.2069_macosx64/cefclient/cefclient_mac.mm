...
- (void)createApplication:(id)object {
  ...
  button_rect.origin.y = window_rect.size.height - URLBAR_HEIGHT +
      (URLBAR_HEIGHT - BUTTON_HEIGHT) / 2 - dTabOffset;
  ...
  if (AppIsOffScreenRenderingEnabled()) {
    ...
    CefRefPtr<OSRWindow> osr_window =
        OSRWindow::Create(&g_main_browser_provider, transparent,
            ...
            CefRect(0, 0, kWindowWidth, kWindowHeight - 30));
    ...
    window_info.SetAsChild(contentView, 0, 0, kWindowWidth, kWindowHeight - 30);
  }
    ...
    NSRect testRect2 = {{50,window_rect.size.height},{window_rect.size.width,30}};
