...
//begin devika can
// Returns true iff the URL matches one of m_Hosts (the set of hosts
// permitted to be displayed in the CAN app).
bool UrlIsFromPermittedHost(const CefURLParts& url_parts) const;

// set m_Hosts from semicolon-separated list
void PopulateHosts(std::string hosts);
// Returns HTML for a page stored as a resource file.
// Text {{message}} or {{location}} will be replaced with text from the
// arguments message and url respectively.

enum ErrorPage {
    ErrorPageNone,
    ErrorPageConnection,
    ErrorPageApplication
};
bool ErrorPageText(std::string& out, ErrorPage errorPage);
bool ErrorPageWithSubstitutions(std::string& out, ErrorPage errorPage,
                                const std::string& url, const std::string& message);
void LaunchTab(CefString& url);
...
//begin devika can
// host names to restrict to (like canapp.com)
typedef std::vector<std::string> HostSet_t;
HostSet_t m_Hosts;
//end devika can
